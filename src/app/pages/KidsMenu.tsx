...
export default function KidsMenu() {
  ...
  const [currentItem, setCurrentItem] = useState<KidsMenuItem | null>(null);
  ...
  const fetchItems = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // If itemId is in URL, find and set that item as current
        if (itemId && data.items && data.items.length > 0) {
          const targetItem = data.items.find((item: KidsMenuItem) => item.id === parseInt(itemId));
          if (targetItem) {
            setCurrentItem(targetItem);
            // Navigate slider to that item
            const targetIndex = data.items.findIndex((item: KidsMenuItem) => item.id === parseInt(itemId));
      ...
      console.error("Error fetching kids menu:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (currentItem) {
      const cartItem = cartItems.find(item => {
        ...
        const currentKey = `${currentItem.id}-Kids Menu`;
  ...
  const handleShare = async () => {
    ...
    const shareUrl = `${window.location.origin}/kids-menu/${currentItem.id}`;
    const shareText = `Check out this amazing Kids Menu deal: ${currentItem.name} - ${currentItem.discountPercentage}% OFF! Rp ${currentItem.finalPrice.toLocaleString("id-ID")}`;
    
    // Try native share API for mobile
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Kids Menu: ${currentItem.name}`,
          text: shareText,
          url: shareUrl,
        });
  ...
  const handleOrderNow = () => {
    ...
    // Navigate to order page with this specific offer
    navigate("/order", {
      state: {
        offerId: `kids-menu-${currentItem.id}`,
        offerData: {
          id: `kids-menu-${currentItem.id}`,
          title: currentItem.name,
          price: currentItem.finalPrice,
      ...
      <div className="min-h-screen bg-background">
        ...
        <header className="bg-primary text-primary-foreground sticky top-0 z-10 shadow-md">
          <div className="max-w-md mx-auto px-4 py-4 flex items-center justify-between">
            ...
            <h1 className="text-lg font-semibold">Kids Menu</h1>
            <div className="w-10"></div>
          </div>
        </header>

        <div className="max-w-md mx-auto px-4 py-12 text-center">
          <p className="text-muted-foreground">No kids menu items available at the moment.</p>
          <button
            onClick={() => navigate("/")}
