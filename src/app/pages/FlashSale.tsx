...
export default function FlashSale() {
  ...
  const fetchItems = async () => {
    try {
      const response = await fetch(`${API_BASE}/flash-sale`, {
        headers: {
      ...
      if (response.ok) {
        ...
        const activeItems = (data.items || []).filter((item: FlashSaleItem) => {
          if (!item.endTime) return true; // No expiry, always active
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Flash Sale: ${currentItem.name}`,
          text: shareText,
          url: shareUrl,
        });
      ...
      <div className="min-h-screen bg-background">
        ...
        <header className="bg-primary text-primary-foreground sticky top-0 z-10 shadow-md">
          <div className="max-w-md mx-auto px-4 py-4 flex items-center justify-between">
            ...
            <h1 className="text-lg font-semibold">Flash Sale</h1>
  ...
  return (
    <div className="min-h-screen bg-background pb-6">
      {/* Header */}
      <header className="bg-gradient-to-r from-pink-600 via-rose-600 to-pink-600 text-white sticky top-0 z-10 shadow-2xl">
        <motion.div 
        ...
        >
          ...
          <div className="flex flex-col items-center">
            <h1 className="text-lg font-semibold">Flash Sale</h1>
            <p className="text-xs text-white/90 font-medium">⚡ Limited Time Offer</p>
          </div>
          <button
            ...
            <div className="flex items-center gap-3 mb-1">
              ...
              {currentItem.endTime && (
                <div className="flex flex-col items-end gap-1">
                  <span className="text-xs text-rose-600 font-semibold">Offer ends in</span>
                  <FlashSaleTimer 
                    endTime={currentItem.endTime} 
                    variant="default"
                    onExpire={() => {
                      // Refresh items when timer expires
                      fetchItems();
                    }}
                  />
                </div>
