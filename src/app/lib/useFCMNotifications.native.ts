  ...
  useEffect(() => {
    ...
    const setup = async () => {
      try {
        ...
        console.log('[fcm] permission check:', permResult);

        if (permResult.notifications !== 'granted' && (permResult as any).receive !== 'granted') {
          console.log('[fcm] requesting permission...');
          const reqResult = await FirebaseMessaging.requestPermissions();
          console.log('[fcm] permission result:', reqResult);
          if (reqResult.notifications !== 'granted' && (reqResult as any).receive !== 'granted') {
